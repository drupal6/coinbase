...
from coinbase.event import Event
from coinbase.api.rest import RestClient, Request
from coinbase.api.websocket import WebsocketClient
from coinbase.trader.constant import (
    Direction,
    Offset,
    Exchange,
    Product,
...
from coinbase.trader.gateway import BaseGateway
from coinbase.trader.object import (
    TickData,
    OrderData,
    TradeData,
    BarData,
...
from coinbase.trader.event import EVENT_TIMER


REST_HOST = "https://api.hbdm.com"
WEBSOCKET_DATA_HOST = "wss://api.btcgateway.pro/option-ws"               # Market Data
WEBSOCKET_TRADE_HOST = "wss://api.hbdm.com/option-notification"    # Account and Order

STATUS_HUOBIO2VT: Dict[int, Status] = {
    3: Status.NOTTRADED,
    4: Status.PARTTRADED,
    5: Status.CANCELLED,
    6: Status.ALLTRADED,
...
ORDERTYPE_VT2HUOBIO: Dict[OrderType, Any] = {
    OrderType.MARKET: "opponent",
    OrderType.LIMIT: "limit",
    OrderType.FOK: "fok",
    OrderType.FAK: "ioc"
}
ORDERTYPE_HUOBIO2VT: Dict[Any, OrderType] = {v: k for k, v in ORDERTYPE_VT2HUOBIO.items()}
ORDERTYPE_HUOBIO2VT[1] = OrderType.LIMIT
ORDERTYPE_HUOBIO2VT[3] = OrderType.MARKET
ORDERTYPE_HUOBIO2VT[4] = OrderType.MARKET
ORDERTYPE_HUOBIO2VT[5] = OrderType.STOP
ORDERTYPE_HUOBIO2VT[6] = OrderType.LIMIT
ORDERTYPE_HUOBIO2VT["lightning"] = OrderType.MARKET
ORDERTYPE_HUOBIO2VT["optimal_5"] = OrderType.MARKET
ORDERTYPE_HUOBIO2VT["optimal_10"] = OrderType.MARKET
ORDERTYPE_HUOBIO2VT["optimal_20"] = OrderType.MARKET

DIRECTION_VT2HUOBIO: Dict[Direction, str] = {
    Direction.LONG: "buy",
    Direction.SHORT: "sell",
}
DIRECTION_HUOBIO2VT: Dict[str, Direction] = {v: k for k, v in DIRECTION_VT2HUOBIO.items()}

OFFSET_VT2HUOBIO: Dict[Offset, str] = {
    Offset.OPEN: "open",
    Offset.CLOSE: "close",
}
OFFSET_HUOBIO2VT: Dict[str, Offset] = {v: k for k, v in OFFSET_VT2HUOBIO.items()}

INTERVAL_VT2HUOBIO: Dict[Interval, str] = {
...
class HuobioGateway(BaseGateway):
    """
    VN Trader Gateway for Huobio connection.
    """
    ...
    def __init__(self, event_engine):
        """Constructor"""
        super().__init__(event_engine, "HUOBIO")

        self.rest_api = HuobioRestApi(self)
        self.trade_ws_api = HuobioTradeWebsocketApi(self)
        self.market_ws_api = HuobioDataWebsocketApi(self)
...
class HuobioRestApi(RestClient):
    """
    HUOBIO REST API
    """
    ...
    def __init__(self, gateway: BaseGateway):
        ...
        self.gateway: HuobioGateway = gateway
    ...
    def query_account(self) -> Request:
        ...
        self.add_request(
            method="POST",
            path="/option-api/v1/option_account_info",
    ...
    def query_history(self, req: HistoryRequest) -> List[BarData]:
        ...
        while True:
            ...
            params = {
                "symbol": ws_symbol,
                "period": INTERVAL_VT2HUOBIO[req.interval],
            ...
            resp = self.request(
                "GET",
                "/option-ex/market/history/kline",
                params=params
            )
    ...
    def cancel_order(self, req: CancelRequest) -> Request:
        ...
        self.add_request(
            method="POST",
            path="/option-api/v1/option_cancel",
...
class HuobioWebsocketApiBase(WebsocketClient):
    """"""

    def __init__(self, gateway):
        """"""
        super(HuobioWebsocketApiBase, self).__init__()
...
class HuobioTradeWebsocketApi(HuobioWebsocketApiBase):
    """"""
    def __init__(self, gateway):
        """"""
...
class HuobioDataWebsocketApi(HuobioWebsocketApiBase):
    """"""
    ...
    def subscribe_data(self, ws_symbol: str) -> None:
        ...
        self.req_id += 1
        req = {
            "sub": f"market.{ws_symbol}.depth.step8",
            "id": str(self.req_id)
        }
        self.send_packet(req)
